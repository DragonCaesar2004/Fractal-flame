...
def test_combine_pixels():
    ...
    assert len(result) == len(
        expected_output
    ), "Должно сохраняться количество уникальных координат"

    for coord, pixel in result.items():
        assert (
            coord in expected_output
        ), f"Координата {coord} не найдена в ожидаемом результате"
...
def test_empty_input():
    # Пустой вход
    input_data: list[dict[ImageCoordsAlias, Pixel]] = []
    result = combine_pixels(input_data)
...
def test_non_overlapping_pixels():
    ...
    assert len(result) == len(
        expected_output
    ), "Длина результата не совпадает с ожидаемым"
